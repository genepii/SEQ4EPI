...
def check_tool(tool_name):
    ...
    if result.returncode != 0:
        logging.error(f"{tool_name} is not installed or not in the PATH.")
        raise FileNotFoundError(f"{tool_name} is not installed or not in the PATH.")
    else:
...
def run_mafft(input_fasta, output_aligned_fasta, tmpdir="/path/to/tmp"):
    ...
    try:
        ...
        logging.error(f"Error running MAFFT: {str(e)}")
        return

# Function to filter sequences that are too short, contain too much ambiguous data, or too many mixed sites
def filter_sequences(input_fasta, output_fasta, log_file, min_length=14000, max_missing_data=150, max_mixed_sites=50):
    ...
    with open(output_fasta, "w") as output_handle:
        for record in SeqIO.parse(input_fasta, "fasta"):
            ...
            # Calculate the number of missing data and mixed sites
            ...
            if len(seq) < min_length:
                ...
                logging.debug(f"Sequence {record.id} rejected: too many missing data ({missing_data_count}).")
                ...
                # Write the valid sequence to the filtered FASTA file
                SeqIO.write(record, output_handle, "fasta")
                valid_sequence_count += 1

            # Add to the list of filtered sequences for a potential report
...
def run_nextalign(input_fasta, output_dir, reference, annotation):
    ...
    logging.info(f"Nextalign completed. Results in: {output_dir}")
...
def run_nextclade(input_fasta, output_dir):
    ...
    nextclade_dataset = "path/to/nextclade_dataset_rsva"  # Update with the actual path to the Nextclade dataset
    ...
    if result.returncode != 0:
        logging.error(f"Nextclade failed with the message: {result.stderr.decode()}")
    else:
        logging.info(f"Nextclade completed. Results in: {output_dir}")
        logging.info(f"Nextclade stdout: {result.stdout.decode()}")
...
def build_phylogenetic_tree(aligned_fasta, output_prefix):
    ...
    try:
        ...
        logging.error(f"Error running IQ-TREE: {str(e)}")
        return
...
def run_treecluster(tree_file, aligned_fasta, output_file, threshold):
    ...
    try:
        ...
        logging.error(f"Error running TreeCluster: {str(e)}")
...
def merge_metadata_with_variants(metadata_file, nextclade_csv_file, nextalign_csv_file, cluster_file, output_file):
    ...
    try:
        ...
        logging.error(f"Error reading the Nextclade CSV file: {e}")
        return
    ...
    try:
        ...
        logging.error(f"Error reading the Nextalign CSV file: {e}")
        return
    ...
    required_columns = ['seqName', 'collection_date', 'location', 'deletions', 'insertions', 'cluster', 'clade', 'qc.overallScore', 'G_clade', 'totalSubstitutions']

    missing_columns = [col for col in required_columns if col not in combined_data.columns]
    if missing_columns:
        logging.error(f"Missing columns after merging: {missing_columns}")
        return
    ...
    logging.info(f"Merged file saved: {output_file}")

def main():
    parser = argparse.ArgumentParser(description='Complete pipeline for alignment, filtering, clustering, and final table generation.')
    parser.add_argument('--input_fasta', required=True, help='FASTA file containing unaligned sequences.')
    ...
    parser.add_argument('--threshold', type=float, required=True, help='Threshold for clustering.')
    parser.add_argument('--metadata_file', required=True, help='CSV file containing metadata.')
    parser.add_argument('--reference', required=True, help='Path to reference file (e.g., reference.fasta).')
    parser.add_argument('--input_annotation', required=True, help='Path to annotation file (e.g., genome_annotation.gff3).')
    ...
    cluster_output_file = f"{args.output_prefix}_{fasta_basename}_clusters.txt"

    tmpdir = "/path/to/tmp"

    run_mafft(args.input_fasta, mafft_output_aligned_fasta, tmpdir)

    if not os.path.exists(mafft_output_aligned_fasta):
        logging.error(f"The file {mafft_output_aligned_fasta} was not generated.")
        return

    filter_sequences(mafft_output_aligned_fasta, filtered_fasta, filtered_log_file)

    if not os.path.exists(filtered_fasta) or os.stat(filtered_fasta).st_size == 0:
        logging.error(f"The filtered file {filtered_fasta} is empty or not generated correctly.")
        return

    run_nextalign(filtered_fasta, nextalign_output_dir, args.reference, args.input_annotation)

    if not os.path.exists(nextalign_aligned_fasta):
        logging.error(f"The file {nextalign_aligned_fasta} does not exist. Ensure Nextalign generates this file.")
        return

    run_nextclade(nextalign_aligned_fasta, nextclade_output_dir)

    build_phylogenetic_tree(filtered_fasta, args.output_prefix)

    if not os.path.exists(tree_file):
        logging.error(f"The file {tree_file} was not generated.")
        return

    run_treecluster(tree_file, filtered_fasta, cluster_output_file, args.threshold)

    if not os.path.exists(cluster_output_file):
        logging.error(f"The file {cluster_output_file} was not generated.")
        return

    merge_metadata_with_variants(args.metadata_file, os.path.join(nextclade_output_dir, 'nextclade.csv'), nextalign_insertions_csv, cluster_output_file, merged_metadata_file)
